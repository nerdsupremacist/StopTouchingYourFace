...
class FaceTouchDetector {
    ...
    private let model = try! VNCoreMLModel(for: MLModel(contentsOf: FaceTouchDetection.urlOfModelInThisBundle))
    ...
    func detect(image: CIImage) throws -> Status {
        ...
        try handler.perform([faceDetectionRequest, faceTouchDetectionRequest], on: image)
...
extension FaceTouchDetector {
    ...
    class Service {
        ...
        func start(device: AVCaptureDevice, frameRate: Double? = nil) {
            _detector = _Detector(device: device, subject: subject, frameRate: frameRate)
        }
...
extension FaceTouchDetector.Service {
    ...
    private class _Detector: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate {
        ...
        init(device: AVCaptureDevice, subject: PassthroughSubject<FaceTouchDetector.Status, Error>, frameRate: Double?) {
            ...
            do {
                ...
                if let connection = output.connection(with: .video) {
                    if connection.isVideoMinFrameDurationSupported {
                        connection.videoMinFrameDuration = frameRate.map { CMTime(seconds: 1 / $0, preferredTimescale: 1) } ?? .invalid
                    }
                }
